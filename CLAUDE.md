...
- **A landing pins filters that cannot be widened.** `/[category]/[landing]` passes `pinnedFilters` to `CatalogPage`, which applies them *after* the query string, hides those dimensions from the sidebar and refuses to write them in `updateParams`. An unknown or draft address is the same 404, and `generateMetadata` says `noindex` there because the streamed response commits a 200 first.
...
- **Breadcrumbs come from one array.** `common/components/Breadcrumbs.tsx` renders the visible trail *and* the `BreadcrumbList` from the same `items`, because they had drifted — the product page showed two crumbs while claiming three.
