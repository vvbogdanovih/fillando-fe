...
- **A swatch is painted from its stops, not from a flag.** `swatchBackground` (`common/components/ColorSwatch.tsx`) is the single rule: one stop is solid, two or more a linear gradient, and `multicolor` a conic one so a rainbow reads as a ring. The admin preview, the variant picker and the storefront filter all call it.
...
**Paying a stuck order and changing the payment method** (TD-0009): the shared pieces live in `common/components/order-payment/` — `PayNowButton`, `ChangePaymentMethodDialog`, `PaymentMethodOptions`, the two API calls and `describePaymentError`. Three rules: (1) the lookup's `liqpay_retry_after_seconds` is the cooldown clock — `null` means no card session was ever opened (offer «Оплатити карткою» at once, no 60 s wait), `n > 0` means render the button disabled with the minutes left, never a click that answers 409; (2) the «Спосіб оплати змінено» view is keyed on `lookup.payment_method`, not on the URL's `payment=LIQPAY`, and is checked **before** the status switch — after a change the URL still says LIQPAY; polling stops when the method is no longer LIQPAY; (3) the change-method button appears only when the backend says `can_change_payment_method: true` (an older backend that omits the field has no endpoint to call), and the compatibility rule for COD/CASH is the one `isPaymentMethodAllowed` in `common/constants/payment.constants.ts` shares with the checkout. Errors carry a `code` in `err.details` — read it, do not parse the message.
