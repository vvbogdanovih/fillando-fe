...
export const feedApi = {
	...
	/**
	 * Synchronous rebuild; 409 while another generation is running. A 2xx can still carry
	 * `ok: false` — a refused publish, not a success.
	 */
	regenerate: () =>
	...
	/** The same origin applied to the path the status reports, so the screen shows a real URL. */
	absoluteUrl: (path: string) => `${API_BASE_URL}${path}`
}
